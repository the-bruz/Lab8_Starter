describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let sdRoot = await item.getProperty('shadowRoot');
    let but = await sdRoot.$('button');
    // Once you have the button, you can click it and check the innerText property of the button.
    await but.click();
    let txt = await but.getProperty('innerText');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    await page.reload();
    let allAreStored = true;
    let items = await page.$$('product-item');
    for (let i = 0; i < items.length; i++){
      let item = items[i];
      let sdRoot = await item.getProperty('shadowRoot');
      let but = await sdRoot.$('button');
      let txt = await (await but.getProperty('innerText')).jsonValue()
      expect(txt).toBe("Remove from Cart")
    }
    // Also check to make sure that #cart-count is still 20
    let count = parseInt(await (await (await page.$('#cart-count')).getProperty('innerText')).jsonValue());
    expect(count).toBe(20);
  }, 10000);
